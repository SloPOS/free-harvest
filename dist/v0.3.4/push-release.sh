#!/usr/bin/env bash
#
# Publish Free Harvest v0.3.4 and mark the withdrawn 0.3.x releases.
#
...
#
#   bash dist/v0.3.4/push-release.sh
#
set -uo pipefail
...
# v0.3 and v0.3.1 are the only withdrawn versions that were ever published.
# v0.3.2 and v0.3.3 existed only as local builds - nothing to mark.
WITHDRAWN=(v0.3.1 v0.3)
...
for f in bootloader.bin partition-table.bin ota_data_initial.bin \
         hr_wifi_adapter.bin RELEASE_NOTES.md DEPRECATE.md; do
...
if gh release view "$TAG" >/dev/null 2>&1; then
  ...
  gh release edit "$TAG" \
    --title "Free Harvest $TAG - fixes the crash in 0.3-0.3.3, adds the trend graph" \
